...
void mode_2(void)
{
    ...
    switch (state) {
        ...
        case 3:
            Keep_Angle_Straight(straight_target_angle, 250);//B-D
            straight_cnt++;
        ...
        // 保持触发瞬间的航向短直行一段距离，避免直接原地转弯压在线上。
            ...
            straight_cnt++;
            if (straight_cnt >= 20) {
                turn_target_angle = normalize_angle(straight_target_angle - 130.0f);
                state = 5;
                straight_cnt = 0;
                stable_cnt = 0;
            }
        ...
        case 6:
            Xunji_Speed();//D-C
            stable_cnt++;
            if (stable_cnt > 50 && (digital(8) || digital(7))) {
                right_black_cnt++;
        ...
        case 8:
            ...
            break;
        case 9:
            Keep_Angle_Straight(straight_target_angle, 250);//停车
            straight_cnt++;
           if (straight_cnt >= 50 && any_black()) {
            lap_count++;
...
void mode_3(void)
{
    ...
    static uint16_t straight_cnt = 0;          // 检测到左侧黑线后的短直行计时。
    static uint16_t cooldown_cnt = 0;          // 转弯后冷却计时，避免立刻重复触发。
    static float straight_target_angle = 0.0f; // 当前直线段的目标航向。
    static float turn_target_angle = 0.0f;     // 需要转到的目标航向。
    static float base_angle = 0.0f;            // 第一条直线启动时锁定的全局基准航向。
    static uint8_t base_angle_valid = 0U;
    static uint16_t lap_count = 0;             // 圈数计数。
    Gyro_Struct *JY61P_Data = get_angle();
    float current_yaw = JY61P_Data->z;

    if (!base_angle_valid && state == 0 && lap_count == 0U) {
    ...
    switch (state) {
        case 0:
            ...
            if (stable_cnt>60 && (digital(1))) {
                ...
                if (left_black_cnt > 0) {
                    straight_target_angle = base_angle;
                    turn_target_angle = normalize_angle(base_angle + 130.0f);
                    state = 2;
                    left_black_cnt = 0;
                    stable_cnt = 0;
                    straight_cnt = 0;
        ...
        case 3:
            Keep_Angle_Straight(straight_target_angle,200);//B-D
            straight_cnt++;
            if (straight_cnt >= 0 && any_black()) {
                left_black_cnt++;
                if (left_black_cnt > 0) {
                    turn_target_angle = normalize_angle(straight_target_angle - 120.0f);
                    state = 5;
                    left_black_cnt = 0;
                    stable_cnt = 0;
                    straight_cnt = 0;
        ...
        case 6:
            Xunji_Speed();//D-C
            stable_cnt++;
            if (stable_cnt >40 && (digital(8) || digital(7))) {
                right_black_cnt++;
                if (right_black_cnt > 0) {
                    straight_target_angle = base_angle;
                    turn_target_angle = normalize_angle(base_angle - 135.0f);
                    state = 8;
                    right_black_cnt = 0;
                    stable_cnt = 0;
                    straight_cnt = 0;
        ...
        case 8:
            ...
            break;
        case 9:
            Keep_Angle_Straight(straight_target_angle, 200);//停车
            straight_cnt++;
           if (straight_cnt >= 0 && any_black()) {
            lap_count++;
